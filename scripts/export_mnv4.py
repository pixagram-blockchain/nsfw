#!/usr/bin/env python
"""
export_mnv4.py — export the trained timm MobileNet NSFW model (from train_nsfw.py)
to ONNX + INT8, VERIFIED against PyTorch, for onnxruntime-web.

Same hardened approach as export_model.py: a LogitsOnly wrapper, an export-
strategy sweep that keeps the FIRST config matching PyTorch within 1e-3, and
static INT8 quantization with calibration. No AvgPool patch is needed — timm
MobileNetV4/V3 heads pool cleanly to ONNX GlobalAveragePool.
...
RUN:
    python scripts/export_mnv4.py                       # uses model/nsfw_mnv4.pt
    python scripts/export_mnv4.py --calib-data data/    # sample calib imgs from dataset
Then:
...
def parse_args():
    ...
    p.add_argument("--calib-count", type=int, default=300)
    return p.parse_args()
...
def main():
    ...
    if calib_files:
        ...
        # verify INT8 didn't collapse vs FP32 on a probe
